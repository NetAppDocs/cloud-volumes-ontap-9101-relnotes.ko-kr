...
클라우드 공급자의 콘솔에서 Cloud Volumes ONTAP 시스템의 구성을 변경하면 지원되지 않는 구성이 생성됩니다. BlueXP에서 생성 및 관리하는 Cloud Volumes ONTAP 리소스에 대한 모든 변경 사항은 시스템 안정성과 BlueXP의 시스템 관리 능력에 영향을 줄 수 있습니다.
